...
const CharacterSelectGrid: Component<Props> = (props) => {
    const buildClasses = (character: Character) => {
        var classes: {[k: string]: boolean} = {
            ...
            "maw:hover:opacity-100": true,

            "maw:border-solid": true,
            "maw:border-[4px]": true,
            "maw:rounded-xl": true,
            "maw:border-[transparent]": props.selectedCharacter?.name !== character.name,
            "maw:border-[red]": props.selectedCharacter?.name === character.name,
            "maw:hover:border-[red]": true,
        };
